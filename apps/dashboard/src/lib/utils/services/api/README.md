...
```typescript
import { apiClient, safeRequest, pathworks } from '$lib/utils/services/api';
...
// Use with Hono RPC
const data = await pathworks.users.$get();
const user = await pathworks.users.$post({ json: { name: 'John' } });
```
